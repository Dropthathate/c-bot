...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    if (!document.getElementById('soma-fonts')) {
      ...
      link.id = 'soma-fonts'; link.rel = 'stylesheet'
    ...
    // ── Cursor ─────────────────────────────────────────────────
    const cd = document.getElementById('soma-cd')
    const cr = document.getElementById('soma-cr')
    ...
    document.querySelectorAll('#soma-root a, #soma-root button').forEach(el => {
    ...
    const onScroll = () => nav && nav.classList.toggle('stuck', window.scrollY > 60)
    ...
    if (canvas) {
      ...
      const blobs = [{x:.72,y:.25,r:340,hue:210},{x:.20,y:.65,r:270,hue:195},{x:.55,y:.75,r:210,hue:225}]
      const drawBlob = (cx,cy,r,hue) => {
        ctx.beginPath()
        for (let i=0;i<=16;i++) {
          const a=(i/16)*Math.PI*2, noise=Math.sin(a*3+t*.7)*35+Math.cos(a*2+t*.5)*22
          const x=cx+Math.cos(a)*(r+noise), y=cy+Math.sin(a)*(r+noise)
          i===0?ctx.moveTo(x,y):ctx.lineTo(x,y)
        }
        ctx.closePath()
        const g=ctx.createRadialGradient(cx,cy,0,cx,cy,r+60)
        g.addColorStop(0,`hsla(${hue},80%,50%,.17)`)
        g.addColorStop(.6,`hsla(${hue},70%,40%,.08)`)
  ...
  // ── Helpers ────────────────────────────────────────────────
  const Step = ({ label, active, current, last }) => (
    <div style={{ flex:1, padding:'10px 12px', textAlign:'center', opacity: active||current ? 1 : 0.35,
      background: current ? 'rgba(0,232,154,0.08)' : active ? 'rgba(0,232,154,0.12)' : 'transparent',
      borderRight: last ? 'none' : '1px solid rgba(255,255,255,0.07)' }}>
      <div style={{ fontSize:'.55rem', fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase',
        color: active||current ? 'var(--grn)' : 'var(--muted)' }}>{label}</div>
    </div>
  )
      ...
      <div id="soma-root">
        ...
        <nav id="soma-nav">
          <a className="n-logo" href="#">
            <img src="./ss.png" alt="SomaSyncAI"
              style={{ height:38, width:'auto', objectFit:'contain', filter:'drop-shadow(0 0 8px rgba(59,158,255,0.4))' }} />
          </a>
        ...
        <section id="hero">
          ...
          <div className="hero-inner">
            <div className="hero-badge"><div className="hero-badge-dot" />Now in Beta — Limited Access</div>
            <h1 className="hero-h1">
              THE <span className="out">GOLD</span><br />
              <span style={{ color:'var(--blue)', textShadow:'0 0 80px var(--blue-glow)' }}>STANDARD</span><br />
              CLINICAL <span className="out">OS</span>
            </h1>
            <div style={{ marginTop:24, fontFamily:"'Syne',sans-serif", fontSize:'clamp(.9rem,1.5vw,1.2rem)',
              fontWeight:700, letterSpacing:'.04em', color:'var(--muted)', opacity:0,
              animation:'fadeUp .8s .7s forwards' }}>The Intelligence of You.</div>
          </div>
              ...
              <div className="dash-body">
                <div className="dash-sidebar">
                  ...
                  {['Live Session','SOAP Notes','Patients','Treatment Memory','ICD-10 Assist','Analytics','Settings'].map((item,i) => (
                    <div key={item} className={`nav-item${i===0?' on':''}`}><div className="ni-dot" />{item}</div>
                  ))}
                </div>
                <div className="dash-main">
                  ...
                  <div style={{ display:'flex', marginBottom:20, border:'1px solid rgba(255,255,255,0.07)', borderRadius:10, overflow:'hidden' }}>
                    <Step label="✓ Intake"      active={true}  last={false} />
                    <Step label="● Calibration" current={true} last={false} />
                    <Step label="Analysis"    last={false} />
                    <Step label="Assessment"  last={false} />
                    <Step label="Generate"    last={false} />
                    <Step label="Live Session" last={true} />
                  </div>
                  <div className="aa-box" style={{ marginBottom:16 }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ display:'flex', gap:8 }}>
                        <div style={{ background:'var(--grn)', color:'#080808', fontSize:'.65rem', fontWeight:700, padding:'6px 14px', borderRadius:100 }}>✓ Consent Given</div>
                        <div style={{ background:'rgba(255,255,255,0.06)', color:'var(--muted)', fontSize:'.65rem', fontWeight:700, padding:'6px 14px', borderRadius:100 }}>View Policy</div>
                      </div>
                    </div>
                  </div>
                  <div className="aa-box">
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div className="aa-t" style={{ marginBottom:10 }}>
                        Say clearly: <b>"AALIYAH, begin session for Sarah Mitchell."</b>
                      </div>
                      <div ref={wf1Ref} className="waveform"
                        style={{ marginTop:0, height:32, border:'none', background:'transparent', padding:0 }} />
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="mq-wrap">
          <div className="mq-track">
            {[...Array(2)].map((_,d) =>
              ['Live In-Ear AI','Automatic SOAP Notes','Voice Charting','Treatment Memory',
               'Gold Standard Documentation','Physical Therapy','Chiropractic',
               'Massage Therapy','Sports Medicine','ICD-10 Assist','AALIYAH.IO Powered'].map((item,i) => (
                <span key={`${d}-${i}`} className="mq-item">{item} <span className="s">◆</span></span>
              ))
            )}
          </div>
        </div>
            ...
            <div className="ds-screen">
              ...
              <div className="ds-inner">
                <div className="ds-side">
                  ...
                  {[['🎧','Live Session',true],['📋','SOAP Notes'],['👤','Patients'],['🧠','Treatment Memory'],['🔬','ICD-10 Assist'],['📊','Analytics'],['⚙️','Settings']].map(([ico,label,on]) => (
                    <div key={label} className={`ds-ni${on?' on':''}`}><span className="ds-ni-ico">{ico}</span>{label}</div>
                  ))}
                </div>
                <div className="ds-center">
                  ...
                  <div style={{ display:'flex', marginBottom:22, border:'1px solid rgba(255,255,255,0.07)', borderRadius:10, overflow:'hidden' }}>
                    {['✓ Intake','✓ Calibration','● Analysis','Assessment','Generate','Live Session'].map((s,i) => (
                      <div key={s} style={{ flex:1, padding:'9px 10px', textAlign:'center', opacity:i>=3?0.3:1,
                        background:i<2?'rgba(0,232,154,0.1)':i===2?'rgba(0,232,154,0.14)':'transparent',
                        borderRight:i<5?'1px solid rgba(255,255,255,0.07)':'none' }}>
                        <div style={{ fontSize:'.52rem', fontWeight:700, letterSpacing:'.08em', textTransform:'uppercase',
                          color:i<3?'var(--grn)':'var(--muted)' }}>{s}</div>
                      </div>
                    ))}
                  </div>
                  <div className="ds-sg">
                    <div className="ds-sc"><div className="ds-sl">Chief Complaint</div><div className="ds-st">Cervical pain and <b>tension headaches.</b> Onset 3 months. Desk worker, 10hr/day.</div></div>
                    <div className="ds-sc"><div className="ds-sl">Relevant History</div><div className="ds-st">No prior surgeries. <b>Hypertension (managed).</b> Previous whiplash 2018.</div></div>
                    <div className="ds-sc" style={{ background:'rgba(255,80,80,0.04)', borderColor:'rgba(255,100,100,0.2)' }}>
                      ...
                      <div className="ds-st">Hypertension noted — <b>avoid aggressive cervical manipulation.</b></div>
                    </div>
                    <div className="ds-sc" style={{ background:'rgba(0,232,154,0.04)', borderColor:'rgba(0,232,154,0.18)' }}>
                      ...
                      <div className="ds-st">Perform <b>cervical orthopedic tests:</b> Spurling's, distraction, Valsalva.</div>
                    </div>
                  </div>
                  <div className="ds-aa">
                    ...
                    <div>
                      ...
                      <div className="ds-aa-t">Health history processed. <b>1 flag raised</b> (hypertension). Cleared to proceed — confirm BP, then begin visual assessment.</div>
                    </div>
                  </div>
                  <div ref={wf2Ref} className="ds-wv" />
                </div>
                <div className="ds-right">
                  <div className="ds-rt">Today's Patients</div>
                  {[
                    ['James R.', '9:00 AM · Cervical / PT\nSession #7 of 12', true],
                    ['Maria S.', '10:30 AM · Lower Back\nSession #3 of 8', false],
                    ['David K.', '12:00 PM · Shoulder / Sports\nNew Patient', false],
                  ].map(([name, info, active]) => (
                    <div key={name} className="ds-pat">
                      <div className="ds-pn">{name}</div>
                      <div className="ds-pi" style={{ whiteSpace:'pre-line' }}>{info}</div>
                      {active && <div className="ds-ptag">● Active Now</div>}
                    </div>
                  ))}
                  <div style={{ marginTop:20, paddingTop:20, borderTop:'1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    {[['Charting Time Saved','47 min'],['Notes Generated','3 / 3'],['ICD-10 Codes','M54.2, M54.5'],['AALIYAH Insights','12 today']].map(([l,v]) => (
                      <div key={l} className="ds-metric"><div className="ds-ml">{l}</div><div className="ds-mv">{v}</div></div>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <section id="how" className="sec">
          ...
          <p className="sec-sub rv d2">From intake to documentation — here's exactly how a session flows.</p>
          <div className="how-grid" style={{ gridTemplateColumns:'repeat(3,1fr)', marginTop:60 }}>
            {[
              { n:'01', title:'Intake Form & Health History', desc:'Client fills out intake while you prepare. A complete health history is the foundation of everything that follows.', sub:'Intake & Preparation' },
              { n:'02', title:'AALIYAH Wakes Up', desc:'Power on the earpiece, open the dashboard. AALIYAH shows consent first — always. Then runs voice calibration.', sub:'Calibration & Consent' },
              { n:'03', title:'Contraindication Analysis', desc:'Reflect the intake history out loud. AALIYAH checks contraindications, flags conditions, suggests orthopedic tests.', sub:'Clinical Analysis' },
              { n:'04', title:'Visual Assessment', desc:'Circle the client. Call out gait, posture, alignment. AALIYAH captures every observation in real time.', sub:'Visual & Postural Assessment', border:true },
              { n:'05', title:'Generate Documentation', desc:'Before bodywork begins, hit Generate. AALIYAH produces a full draft. You review and verify — AI assists, clinician decides.', btn:true, border:true },
              { n:'06', title:'Document Live During Bodywork', desc:'Session begins. Voice-document tissue response, adjustments, outcomes in real time. Walk out with complete notes.', sub:'Live Treatment Session', border:true },
            ].map(s => (
              <div key={s.n} className="how-card rv" style={s.border ? { borderTop:'1px solid rgba(255,255,255,0.06)' } : {}}>
                ...
                {s.btn && <div style={{ marginTop:16, display:'inline-flex', alignItems:'center', gap:7, background:'rgba(0,232,154,0.1)', border:'1px solid rgba(0,232,154,0.25)', color:'var(--grn)', fontSize:'.62rem', fontWeight:700, letterSpacing:'.08em', textTransform:'uppercase', padding:'5px 14px', borderRadius:100 }}>⚡ Generate Documentation</div>}
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="roadmap" className="sec">
          ...
          <div className="road-grid">
            {[
              { phase:'Phase 01 — Q1 2026', title:'Clinical Documentation MVP', live:true,
                items:['Live in-ear AI guidance','Automatic SOAP notes','Voice charting','Treatment memory','ICD-10 code assist','Dashboard & chat'] },
              { phase:'Phase 02 — Q3 2026', title:'Intelligence Expansion', date:'Coming Q3 2026',
                items:['SomaSphere 3D visualizer','SyncLearn in-ear education','EHR integrations','Mobile app (iOS)','Practice dashboard','Billing code assist'] },
              { phase:'Phase 03 — Q4 2026', title:'Wearable + Population Layer', date:'Coming Q4 2026',
                items:['Wearable device integration','Real-time biometrics','Population analytics','Outcome tracking','Referral insights','Pattern reporting'] },
              { phase:'Phase 04 — 2027', title:'Predictive Care OS', date:'Coming 2027',
                items:['Predictive care AI','Re-injury risk flags','Personalised care paths','Multi-location enterprise','API for EHR vendors','International expansion'] },
            ].map((r,i) => (
              <div key={r.phase} className={`road-card rv d${i+1}`}>
                ...
                <ul className="road-items">{r.items.map(item => <li key={item}>{item}</li>)}</ul>
          ...
          <div className="inv-grid">
            <div className="inv-stats">
              {[['$28B','Global TAM'],['1.2M','US Practitioners'],['87%','Charting Reduction'],['$2.5M','Seed Round']].map(([n,l],i) => (
                <div key={l} className={`inv-stat rv d${i+1}`}><div className="inv-n">{n}</div><div className="inv-l">{l}</div></div>
              ))}
            </div>
            <div>
              <div className="why-list">
                {[
                  ...
                  <div key={t} className={`why-item rv d${i+1}`}>
        ...
        <section id="cta">
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:1, background:'rgba(255,255,255,0.08)',
            border:'1px solid rgba(255,255,255,0.08)', maxWidth:860, margin:'0 auto 48px', borderRadius:12, overflow:'hidden' }}>
            {[
              ['01','Use SomaSyncAI','Join beta. Use it in your practice. Experience the difference hands-free documentation makes.'],
              ['02','Give Feedback','Share honest feedback on what works, what doesn\'t, and what you wish it could do.'],
              ['03','Refer One Therapist','Refer one colleague who beta tests and refers someone too. Complete all 3 — get lifetime access.'],
            ].map(([n,t,d],i) => (
              <div key={n} style={{ padding:'28px 24px', background:'rgba(255,255,255,0.03)', textAlign:'center',
                borderLeft: i>0 ? '1px solid rgba(255,255,255,0.08)' : 'none' }}>
                <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'2rem', color:'var(--grn)', marginBottom:8 }}>{n}</div>
                <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:'.88rem', marginBottom:8 }}>{t}</div>
                <div style={{ fontSize:'.78rem', lineHeight:1.6, color:'var(--muted)' }}>{d}</div>
              </div>
            ))}
          </div>
          ...
          <div className="foot-top">
            <div>
              <div className="foot-brand" style={{ marginBottom:16 }}>
                <img src="./ss.png" alt="SomaSyncAI"
                  style={{ height:44, width:'auto', objectFit:'contain', filter:'drop-shadow(0 0 10px rgba(59,158,255,0.35))' }} />
              </div>
              <div className="foot-tag">The gold standard AI OS for manual therapists. Live in-ear guidance. Zero documentation debt.</div>
            </div>
            <div>
...
// ── CSS ────────────────────────────────────────────────────────────────────
const CSS = `
#soma-root,#soma-root *,#soma-root *::before,#soma-root *::after{box-sizing:border-box;margin:0;padding:0}
#soma-root{--bg:#080808;--ink:#f0ede8;--muted:rgba(240,237,232,0.45);--dim:rgba(240,237,232,0.18);--grn:#00e89a;--blue:#3b9eff;--blue2:#1a7ee0;--blue-glow:rgba(59,158,255,0.25);--r:clamp(24px,5vw,80px);scroll-behavior:smooth;background:var(--bg);color:var(--ink);font-family:'Manrope',sans-serif;overflow-x:hidden;-webkit-font-smoothing:antialiased;cursor:none}
#soma-cd,#soma-cr{position:fixed;border-radius:50%;pointer-events:none;z-index:9999;transform:translate(-50%,-50%)}
#soma-cd{width:8px;height:8px;background:#00e89a}
#soma-cr{width:34px;height:34px;border:1px solid rgba(0,232,154,0.5);transition:width .2s,height .2s}
...
#soma-root .n-logo{font-family:'Syne',sans-serif;font-weight:800;font-size:.95rem;color:var(--ink);text-decoration:none;display:flex;align-items:center}
...
#soma-root .n-cta{font-family:'Syne',sans-serif;font-size:.78rem;font-weight:700;background:var(--blue);color:#fff;padding:11px 22px;border-radius:100px;text-decoration:none;transition:background .2s,transform .15s}
#soma-root .n-cta:hover{background:var(--blue2);transform:scale(1.04)}
...
#soma-root #hero canvas{position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:0}
...
#soma-root .dd1{background:#ff5f57}#soma-root .dd2{background:#ffbd2e}#soma-root .dd3{background:#28c840}
...
#soma-root .nav-item{padding:9px 12px;border-radius:8px;font-size:.78rem;color:var(--muted);display:flex;align-items:center;gap:10px}
...
#soma-root .dash-title{font-family:'Syne',sans-serif;font-weight:700;font-size:1rem}
...
#soma-root .sec-tag::before{content:'';display:block;width:24px;height:1.5px;background:var(--blue)}
...
#soma-root .rv.d1{transition-delay:.1s}#soma-root .rv.d2{transition-delay:.2s}#soma-root .rv.d3{transition-delay:.3s}#soma-root .rv.d4{transition-delay:.4s}
...
#soma-root .prob-n{font-family:'Syne',sans-serif;font-weight:800;font-size:4.5rem;color:var(--blue);letter-spacing:-.04em;line-height:1;margin-bottom:12px}
...
#soma-root .feat-title{font-family:'Syne',sans-serif;font-weight:700;font-size:1rem;margin-bottom:10px}
...
#soma-root .ds-cth{font-family:'Syne',sans-serif;font-weight:700;font-size:1.05rem}
...
#soma-root .ds-rt{font-family:'Syne',sans-serif;font-weight:700;font-size:.82rem;margin-bottom:18px;color:var(--muted)}
...
#soma-root .how-t{font-family:'Syne',sans-serif;font-weight:700;font-size:.98rem;margin-bottom:12px}
...
#soma-root .road-t{font-family:'Syne',sans-serif;font-weight:700;font-size:1rem;margin-bottom:16px;line-height:1.2}
...
#soma-root .inv-n{font-family:'Syne',sans-serif;font-weight:800;font-size:3rem;letter-spacing:-.04em;color:var(--blue);line-height:1}
...
#soma-root .deck-link{display:inline-flex;align-items:center;gap:10px;font-family:'Syne',sans-serif;font-weight:700;font-size:.88rem;background:var(--blue);color:#fff;padding:15px 32px;border-radius:100px;text-decoration:none;margin-top:36px;transition:background .2s,transform .15s}
#soma-root .deck-link:hover{background:var(--blue2);transform:translateY(-3px)}
...
#soma-root .cta-h .g{color:var(--blue)}
...
#soma-root .cta-b1{font-family:'Syne',sans-serif;font-size:.92rem;font-weight:700;background:var(--blue);color:#fff;padding:17px 38px;border-radius:100px;text-decoration:none;transition:background .2s,transform .15s}
#soma-root .cta-b1:hover{background:var(--blue2);transform:translateY(-3px)}
