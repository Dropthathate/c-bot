...
voiceRouter.post("/generate-soap", requireAuthenticatedUser, async (request: AuthenticatedRequest, response, next) => {
  try { const { transcript } = transcriptBody.parse(request.body); const note = await generateSoapNote(transcript); return response.status(200).json({ note, review: reviewSessionTranscript(transcript) }); } catch (error) { return next(error); }
});
