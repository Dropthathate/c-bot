...
export default function SoapGenerator() {
  ...
  const handleCopy = () => {
    ...
    const text = [
      ...
      `ASSESSMENT\n${soap.assessment}`,
      `PLAN\n${soap.plan}`,
      soap.icd10?.length ? `ICD-10 (reference only — confirm with prescribing provider)\n${soap.icd10.map(c => `${c.code} — ${c.description}`).join("\n")}` : "",
      // CPT display removed pending AMA licensing — do not re-enable without a license.
            ...
            <div className="card soap-result">
              ...
              {soap.icd10?.length > 0 && (
                <div className="soap-section">
                  <span className="soap-section-label soap-icd">ICD-10 Codes — Reference Only</span>
                  <div style={{ fontSize: '.72rem', color: 'var(--orange)', marginBottom: 8 }}>
                    ⚠ Not a diagnosis. If your scope of practice does not include
                    diagnosing, confirm the correct code with the client's
                    prescribing provider before use on any claim.
                  </div>
              ...
              {/* CPT code display removed pending AMA licensing. Do not re-enable
                  without a distributor license from the AMA — CPT is copyrighted
                  content and requires a license regardless of end-user credentials. */}
