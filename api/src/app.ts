...
import { assertCsrf, assertTrustedBrowserRequest, clearCookieSession, establishCookieSession, requireAuthenticatedUser, sessionSummary, type AuthenticatedRequest } from "./auth.js";
...
const logger = pino({
  ...
  redact: ["req.headers.authorization", `req.cookies.${config.SESSION_COOKIE_NAME}`, `req.cookies.${config.CSRF_COOKIE_NAME}`, "req.body", "res.headers"]
});

app.set("trust proxy", 1);
app.use(pinoHttp({ logger }));
app.use(helmet({ crossOriginResourcePolicy: { policy: "same-site" }, crossOriginOpenerPolicy: { policy: "same-origin-allow-popups" }, contentSecurityPolicy: false }));
app.use(cors({
...
app.use("/api/v1/calendar", assertTrustedBrowserRequest, calendarRouter);
