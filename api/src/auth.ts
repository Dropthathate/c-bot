...
function signedCsrfToken(sessionToken: string, nonce = crypto.randomBytes(32).toString("base64url")) {
...
function hasValidCsrfToken(token: string | undefined, sessionToken: string) {
  if (!token) return false;
  const parts = token.split(".");
  if (parts.length !== 2 || !parts[0] || !parts[1] || !/^[A-Za-z0-9_-]{32,}$/.test(parts[1])) return false;
  const expected = signedCsrfToken(sessionToken, parts[1]);
...
function allowedFetchSite(request: Request) {
...
export function assertAllowedOrigin(request: Request, response: Response, next: NextFunction) {
  if (!allowedOrigin(request.header("origin"))) {
    return response.status(403).json({ error: { code: "ORIGIN_NOT_ALLOWED", message: "This origin is not permitted." } });
  }
  return next();
}

export function assertTrustedBrowserRequest(request: Request, response: Response, next: NextFunction) {
  if (!allowedOrigin(request.header("origin")) || !allowedFetchSite(request)) {
...
export function assertCsrf(request: AuthenticatedRequest, response: Response, next: NextFunction) {
  ...
  const tokenMatches = typeof cookie === "string" && typeof header === "string" && constantTimeEqual(cookie, header);
  if (!sessionToken || !tokenMatches || !hasValidCsrfToken(cookie, sessionToken)) {
...
export async function authenticateWebSocketRequest(request: IncomingMessage): Promise<{ payload: JWTPayload; sessionToken: string }> {
  if (!allowedOrigin(request.headers.origin)) throw new Error("ORIGIN_NOT_ALLOWED");
  ...
  const csrfProtocol = requestedProtocols.find((value) => value.startsWith(csrfProtocolPrefix));
  const csrfToken = csrfProtocol?.slice(csrfProtocolPrefix.length);
  if (!hasValidCsrfToken(csrfToken, token)) throw new Error("CSRF_INVALID");
