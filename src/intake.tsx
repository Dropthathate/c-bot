    ...
    <div className="min-h-screen bg-background p-8 text-foreground">
      <div className="max-w-4xl mx-auto">
        <header className="mb-12">
          <h1 className="text-xs uppercase tracking-[0.2em] text-muted-foreground font-bold mb-2">Clinical Intake</h1>
          ...
          {icd10Codes.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground mt-1">{item.desc}</div>
              </div>
            );
          })}
        </div>

        <footer className="mt-20 flex justify-between items-center border-t border-border pt-8">
          <div className="text-[10px] text-muted-foreground uppercase tracking-widest">
            {selectedCodes.length} Codes Selected
          </div>
          <button className="text-xs font-bold uppercase tracking-[0.3em] text-primary hover:opacity-80 transition-opacity">
            Initialize Session →
          </button>
        </footer>
      </div>
