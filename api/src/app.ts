...
const logger = pino({
  ...
  redact: [
    ...
    `req.cookies.${config.CSRF_COOKIE_NAME}`,
    "req.body",
    "res.headers"
  ]
});
...
// In AWS, terminate TLS at API Gateway/ALB and forward X-Forwarded-Proto.
...
// Secure Space session bridge: Supabase Bearer → HttpOnly cookie + CSRF
...
app.get("/api/v1/auth/session", requireAuthenticatedUser, (request: AuthenticatedRequest, response) =>
  response.status(200).json(sessionSummary(request)));
...
app.use((error: unknown, request: express.Request, response: express.Response, _next: express.NextFunction) => {
  ...
  if (error instanceof multer.MulterError) return response.status(400).json({ error: { code: "INVALID_UPLOAD", message: error.message } });
