...
A new c-bot public Config variable form is open for the verified Production project-URL fallback. No URL value has been entered or saved yet.
A public Config variable named `VITE_SUPABASE_PROJECT_URL` is staged for c-bot Production. Its verified project URL value has not yet been entered or saved.
The verified public Config variable `VITE_SUPABASE_PROJECT_URL=https://ucqprtpuuyflnxjmatwo.supabase.co` was successfully added to c-bot Production. A fresh deployment is now required to embed both the project URL and publishable-key fallbacks.
A fresh Production redeploy was created after both verified public Supabase fallback variables were added. The live bundle will be checked after deployment completion.
The refreshed live beta bundle now embeds the verified Supabase project URL, publishable key, and beta-lead capture path. Browser-based synthetic signup verification is currently blocked only by the site’s Cloudflare human-verification challenge.
Direct functional verification used one clearly marked synthetic address. The raw diagnostic request initially omitted the client’s `public` schema profile and returned `PGRST205` for `api.beta_leads`; the corresponding request with `Content-Profile: public` returned HTTP 201 with `return=minimal`, matching the deployed Supabase client’s public-schema insert path. The correct SomaSyncAi_2025 project SQL Editor opened for a targeted, read-only lookup, but its browser editor did not register injected SQL for execution and displayed `query: Too small: expected string to have >=1 characters`; this does not negate the successful HTTP 201 insert response.
